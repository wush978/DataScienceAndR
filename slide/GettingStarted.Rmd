...
opts_chunk$set(echo = FALSE, cache=FALSE, comment="", cache.path = ".cache-EDA/")
...
.tmp <- strsplit(speakers[1], "")[[1]] %>% split(rep(1:15, each = 30)) %>%
...
.tmp <- strsplit(tmWordsVec[1], "")[[1]] %>% split(rep(1:15, each = 30)) %>%
  sapply(paste, collapse = "")
names(.tmp) <- NULL
cat(.tmp, sep = "\n")
```
...
- Mac: 請至 [CRAN](http://cran.csie.ntu.edu.tw) 上下載 R3.2 以上的安裝程式。
    ...
    - 此影片由中華R軟體學會的李明昌老師提供
- Ubuntu: Ubuntu 14.04 的使用者可以在 bash 執行以下指令來安裝學習環境。如果是其他版本的使用者，請修改上述 trusty 的部份。
...
請在期限內完成作業之後，繳交該檔案到ceiba。
- Bug 回報：<https://github.com/wush978/DataScienceAndR/issues>
    - 看不懂敘述
    - 有錯字、程式卡住...

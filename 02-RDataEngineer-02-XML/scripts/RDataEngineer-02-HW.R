...
# 這個段落的程式碼，主要是先幫助同學觀察資料，不包含在作答的檢查範圍中。
if (FALSE) {
  # 我們先來看看政府電子採購網<http://web.pcc.gov.tw>所爬下來的決標資料。
  # 這個檔案的路徑已經設定到tender_path了。首先，請同學先用`readLines`來看這個檔案的前100行。

  readLines(tender_path, n = 100)

  # 對於這樣的內容，要直接處理是很具挑戰性的，
  # 但是我們可以先透過瀏覽器，來觀察這個HTML文件所夾帶的內容。
...
# 在Windows系統上的機器，必需要額外告訴R 說這段文字為UTF-8編碼。
Encoding(ths_text) <- "UTF-8"

# 請拿ths_text和 "　廠商名稱" 進行比較。
is_target <- {
  # 請在這裡填寫你的程式碼。
}
...
# 我們可以利用 `[`，從ths中選出那些值為 "　廠商名稱" 的xml_nodeset。
ths2 <- ths[is_target]
